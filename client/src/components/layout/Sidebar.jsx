...
const baseNavigation = [
  ...
  { name: 'Attendance', href: '/attendance', icon: ClipboardDocumentListIcon, gradient: 'from-teal-500 to-emerald-500' },
  { name: 'Reports', href: ROUTES.REPORTS, icon: ChartBarIcon, gradient: 'from-indigo-500 to-purple-500' },
  { name: 'My History', href: '/student/history', icon: ClockIcon, gradient: 'from-orange-500 to-red-500' },
]
...
const adminOnlyNavigation = [
  ...
  { name: 'Counsellors', href: '/counsellors', icon: UserGroupIcon,   gradient: 'from-pink-500 to-rose-500' },
  { name: 'Security',    href: '/security',    icon: ShieldCheckIcon, gradient: 'from-orange-500 to-red-500' },
]

const counsellorNavigation = [
  { name: 'Dashboard',   href: '/counsellor/dashboard', icon: HomeIcon,         gradient: 'from-blue-500 to-cyan-500' },
  { name: 'My Students', href: '/students',              icon: UserGroupIcon,    gradient: 'from-green-500 to-emerald-500' },
  { name: 'Outpasses',   href: '/outpass-requests',      icon: DocumentTextIcon, gradient: 'from-purple-500 to-pink-500' },
]
...
export default function Sidebar({ onLogout, mobile }) {
  ...
  // ── Declare ALL role booleans first, before any usage ──────────────────────
  const isAdmin      = user?.role === 'admin'
  const isHod        = user?.role === 'hod'
  const isCounsellor = user?.role === 'counsellor'

  // Build navigation list based on role
  ...
  // Ensure HODs see the Students link
  if (isHod) {
  ...
  // Paths to hide per role
  const studentHiddenPaths    = [ROUTES.STUDENTS, ROUTES.OUTPASS_REQUESTS, ROUTES.REPORTS, '/attendance']
  const securityHiddenPaths   = [ROUTES.REPORTS, '/student/history']
  const counsellorHiddenPaths = [ROUTES.REPORTS, '/student/history', '/attendance', ROUTES.OUTPASS_REQUESTS]
      ...
      <div className="flex flex-col flex-1 min-h-0">
        ...
        >
          <div className="relative">
            <div className="absolute inset-0 bg-gradient-to-r from-blue-600 to-purple-600 rounded-xl blur-md opacity-75" />
        ...
        <nav className="flex-1 px-4 py-6 space-y-1 overflow-y-auto hide-scrollbar">
          {navigation.map((item) => {
            ...
            if (user?.role === 'security'   && securityHiddenPaths.includes(item.href))   return null
            if (user?.role === 'counsellor' && counsellorHiddenPaths.includes(item.href)) return null
            ...
            return (
              <Motion.div
                ...
                whileTap={{ scale: 0.98 }}
              >
                <Link to={item.href} className="group relative block">
                  {/* Active background */}
                  {isActive && (
                    <Motion.div
                      layoutId="activeTab"
                  ...
                  <div className={`relative flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 ${
                    ...
                    <div className={`relative ${isActive ? 'scale-110' : ''} transition-transform duration-200`}>
                      {isActive && (
                        <div className={`absolute inset-0 bg-gradient-to-r ${item.gradient} rounded-lg blur-md opacity-50`} />
                      )}
                      <Icon
                        className={
                          isActive
                            ? `relative h-5 w-5 bg-gradient-to-r ${item.gradient} text-transparent bg-clip-text`
                            : 'relative h-5 w-5'
                        }
                      />
                    </div>
                    ...
                    {/* Hover fill */}
                    {!isActive && (
                      <div className="absolute inset-0 bg-slate-100 dark:bg-slate-700/30 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-200 -z-10" />
                    )}
                  </div>
                </Link>
              </Motion.div>
        ...
        <div className="absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t from-slate-100/50 dark:from-slate-900/50 to-transparent pointer-events-none" />
      </div>
    </Motion.aside>
  )
}
