...
import { USER_ROLES, USER_STATUS, HOSTEL_TYPES, DEPARTMENTS, HOSTEL_BLOCKS, ALL_BLOCKS, VALIDATION, COURSES } from '../../constants'
...
export default function AdminDashboard() {
  ...
  const [hostelBlocksList, setHostelBlocksList] = useState(ALL_BLOCKS)
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      setStats((prev) => ({
        ...
        activeUsers:     s.activeUsers     || 0,   // was missing
        totalOutpasses:  s.totalOutpasses  || 0,   // was missing
        pendingOutpasses: s.pendingOutpasses || 0,
        securityCount: s.securityCount || 0,
        counsellorCount: s.counsellorCount || 0, 
  ...
  const roleDistribution = [
    ...
    { role: 'Counsellors', count: stats.counsellorCount, color: 'from-pink-500 to-rose-500' },
          ...
          <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  >
                    ...
                    {(selectedUser?.student?.hostelType
                      ? HOSTEL_BLOCKS[selectedUser.student.hostelType]
                      : hostelBlocksList
                    ).map(b => (
            ...
            {selectedUser?.role === USER_ROLES.WARDEN && (
              ...
              >
                ...
                >
                  <option value="">Select Block</option>
                  {(selectedUser?.warden?.hostelType
                    ? HOSTEL_BLOCKS[selectedUser.warden.hostelType]
                    : hostelBlocksList
                  ).map(b => (
                    <option key={b} value={b}>{`Block ${b}`}</option>
                  ))}
  ...
  <Motion.div
    ...
    className="space-y-4 pt-4 border-t border-slate-200 dark:border-slate-700"
  >
    <h4 className="font-semibold text-slate-900 dark:text-white">Counsellor Details</h4>
    <Select
      label="Department"
      glassmorphic
      required
      value={selectedUser?.counsellor?.department || ''}
      onChange={(e) =>
        setSelectedUser(prev => ({
          ...(prev || {}),
          counsellor: { ...(prev?.counsellor || {}), department: e.target.value }
        }))
      }
    >
      <option value="">Select Department</option>
    ...
    <Select
      label="Hostel Type (if applicable)"
      glassmorphic
      value={selectedUser?.counsellor?.hostelType || ''}
      onChange={(e) =>
        setSelectedUser(prev => ({
          ...(prev || {}),
          counsellor: { ...(prev?.counsellor || {}), hostelType: e.target.value }
        }))
      }
    >
              ...
              <Button
                ...
                className="flex-1"
                    onClick={() => {
                      ...
                      if (selectedUser.role === 'counsellor') {
                const c = selectedUser.counsellor || {}
                if (!c.department) {
                  toast.error('Select a department for the counsellor')
                  return
                }
                const createPayload = {
                  ...selectedUser,
                  department: c.department,
                  ...(c.hostelType ? { hostelType: c.hostelType } : {}),
                }
                // Remove the nested counsellor object — server expects flat fields
                delete createPayload.counsellor
                handleCreateUser(createPayload)
                return
              }
